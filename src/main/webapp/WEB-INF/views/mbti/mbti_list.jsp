...
.mbti-container {
	display: grid;
	grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); /* 너비 기준으로 자동 줄바꿈 */
	gap: 30px;
}
...
.mbti-container .card {
	...
	justify-content: center;  /* 수직 중앙 정렬 (높이 지정 필요) */
	height: 300px; /* 카드 높이 설정 필요 */
	text-align: center;
	border-radius: 20px;
...
<c:forEach items="${ MBTIs }" var="row" varStatus="loop">
	            <div class="card">
	            	<a href="/mbtiView.do?idx=${row.mbtiIdx }">
		                <div class="image">
