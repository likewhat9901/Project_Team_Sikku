	...
	<div class="login-wrapper">
		...
		<div class="form-container">
			...
			<c:if test="${empty userid}" var="loginResult">
				<c:if test="${param.error != null}">
					<p class="error-message">
						Login Error! <br />${errorMsg}</p>
				</c:if>

				<form action="/myLoginAction.do" method="post">
					<label for="userid">아이디</label> <input type="text" id="userid"
						name="my_id" placeholder="아이디 입력" required> <label
						for="userpwd">비밀번호</label> <input type="password" id="userpwd"
						name="my_pass" placeholder="비밀번호 입력" required>
			...
			<c:if test="${not loginResult}">
				<p class="welcome-message">${username}님방문해주셔서 감사합니다</p>
